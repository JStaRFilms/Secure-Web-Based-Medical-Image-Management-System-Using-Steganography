...
import { Lock, Unlock, Shield, FileLock, FileKey, ArrowRight, ArrowUp } from 'lucide-react'
import { getStats, getRecentActivity } from '@/actions/audit'
        ...
        <div className="p-8 max-w-6xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-6 rounded-xl shadow-sm">
                    ...
                    <p className="text-3xl font-bold font-mono">{stats.encryptionCount}</p>
                    <p className="text-xs text-green-500 mt-1 flex items-center gap-1">
                        <ArrowUp className="w-3 h-3" /> System Active
                    </p>
                </div>
                <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-6 rounded-xl shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                    ...
                    <p className="text-3xl font-bold font-mono">{stats.decryptionCount}</p>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <Link
                    href="/encryption-lab"
                    className="flex items-center p-6 bg-blue-600 text-white dark:bg-cyan-600 rounded-xl hover:bg-blue-700 dark:hover:bg-cyan-700 transition-all shadow-lg shadow-blue-500/20 dark:shadow-cyan-500/20 group"
                >
                ...
                <Link
                    href="/decryption-lab"
                    className="flex items-center p-6 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800 transition-all group"
                >
            ...
            <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                ...
                <div className="divide-y divide-slate-100 dark:divide-zinc-800">
                    {activities.length === 0 ? (
                        <div className="p-6 text-center text-sm text-slate-500">No activity recorded yet.</div>
                    ) : (
                        activities.map((log) => (
                            <div key={log.id} className="px-6 py-4 flex items-center justify-between text-sm">
                                <div className="flex items-center gap-3">
                                    <div className={`w-2 h-2 rounded-full ${log.operation === 'ENCRYPT' ? 'bg-blue-500' : 'bg-purple-500'}`}></div>
                                    <span className="font-mono text-slate-600 dark:text-zinc-400">{log.operation}</span>
                                    <span className="text-slate-900 dark:text-zinc-200">{log.details}</span>
                                </div>
